...
const Editorder = ({ setAlert }) => {
  ...
  const { _id, customer, description, picturepath, date, price, hours, item1, item1_l, item1_w, item2, item2_l, item2_w,canvas } = order;
  ...
  const send = event => {
    ...
    order.canvas = childRef.current.getSaveData()
    axios.patch(`http://localhost:3001/order/${id}`, order)
      .then(function (res) {
    ...
    <div className="container py-4">
      <div className="container">
        <div className="w-75 mx-auto shadow p-5">
          ...
          <h2 className="text-center mb-4">  {order.id} </h2>
          <div className="App">
